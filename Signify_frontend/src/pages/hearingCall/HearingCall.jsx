  ...
  useEffect(() => {
    ...
    const triggerVideoCall = async () => {
      try {
        ...
        console.log("TEST :- ",response.data === config);
        ...
        agoraEngine.on("user-published", async (user, mediaType) => {
          ...
          // console.log(user.uid,"  *******    " ,config.uid)
        ...
        agoraEngine.on("user-unpublished", async (user) => {
          if (channelParameters.remoteAudioTrack && channelParameters.remoteVideoTrack) {
            // Unsubscribe from the remote user's audio and video tracks
            await agoraEngine.unsubscribe(channelParameters.remoteAudioTrack);
            await agoraEngine.unsubscribe(channelParameters.remoteVideoTrack);
  ...
  return (
    <div className="min-h-screen w-screen bg-[#202124] pt-6 px-16 relative overflow-hidden">
      ...
      >
        ...
        quod totam alias, excepturi necessitatibus dolores animi nisi distinctio
        sunt pariatur. Ex mollitia obcaecati et, magni esse atque corrupti
        incidunt officia animi.
      </div>
    </div>
  );
};
