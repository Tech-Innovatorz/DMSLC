    ...
    const getMeetToken = async () => {
      ...
      try {
        const response = await axios.get(
          "http://localhost:8800/generateToken",
          {}
        );
  ...
  return (
    <div className="bg-blue-300 min-h-screen w-screen py-4 flex justify-between items-center px-28">
      <div className="inline-block w-[40rem]">
        ...
        <p className="text-4xl"> now free for everyone</p>
        <br />
        <p className="text-2xl font-thin">
          We reengineer the way of commumnication for everyone, now inclusive of
          deaf people. In our meet, we make it inclusive to all
        </p>
        <div className="flex space-x-2 items-center my-12">
          <button
            className="w-52 h-12 bg-blue-600 text-lg font-semibold text-white rounded-lg"
            onClick={instantMeetHandler}
          >
            <TiVideo className="inline-block mr-2 h-7 w-7 fill-white" />
            create instant meet
          </button>
          <div>
            <input
            ...
            />
            <button
              className="disabled:text-gray-500 font-semibold text-lg"
